...
class DropDownHelper
{
    ...
    public static function renderIndustryDropdown($selectedIndustry = null, $selectedSubcategory = null)
    {
        ...
        foreach ($industries as $industry => $subcategories) {
            $isSelected = $industry == $selectedIndustry ? 'selected' : '';
            $html .= '<option value="' . $industry . '" ' . $isSelected . '>' . $industry . '</option>';
        }

        $html .= '</select>';
        $html .= '
            <script>
                const industryData = ' . json_encode($industries) . ';
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    function loadSubcategories(industry) {
                        subCategoryDropdown.innerHTML = "<option value=\'\'>Select Company Sub Category</option>";
                        if (industryData[industry]) {
                            industryData[industry].forEach(function(subCategory) {
                    ...
                    loadSubcategories(industryDropdown.value); // Preload subcategories on page load if industry is selected
                });
            </script>
        ';
    ...
    public static function renderSubcategoryDropdown($selectedIndustry = null, $selectedSubcategory = null)
    {
        $html = '<select name="company_sub_category" id="company_sub_category" class="form-select">';
        $html .= '<option value="">Select Company Sub Category</option>';
        $html .= '</select>';
    ...
    public static function renderBusinessTypeDropdown($selectedBusinessType = null)
    {
        ...
        // Generate the HTML for business type dropdown
    ...
    public static function renderBusinessContributionsToMuslimCommunityDropdown($selectedContribution = null)
    {
        $contributions = [
            ...
            'Support for Muslim Organizations' => 'Support for Muslim Organizations',
            'Others' => 'Others',
        ];
    ...
    {
        $affiliations = [
            'Council on American-Islamic Relations (CAIR)' => 'Council on American-Islamic Relations (CAIR)',
            'Islamic Society of North America (ISNA)' => 'Islamic Society of North America (ISNA)',
            'American Muslim Alliance (AMA)' => 'American Muslim Alliance (AMA)',
            'American Muslim Council (AMC)' => 'American Muslim Council (AMC)',
            'Muslim Public Affairs Council (MPAC)' => 'Muslim Public Affairs Council (MPAC)',
            'Zakat Foundation of America' => 'Zakat Foundation of America',
            'Islamic Relief USA' => 'Islamic Relief USA',
            'American Arab Anti-Discrimination Committee (ADC)' => 'American Arab Anti-Discrimination Committee (ADC)',
            'Yaqeen Institute for Islamic Research' => 'Yaqeen Institute for Islamic Research',
            'Islamic Circle of North America (ICNA)' => 'Islamic Circle of North America (ICNA)',
            'Others' => 'Others',
        ];

        // Generate the HTML for the affiliation dropdown
    ...
    public static function renderSubcategoryDropdownForUser($selectedIndustry = null, $selectedSubcategory = null)
    {
        $html = '<select name="sub_category_to_connect" id="sub_category_to_connect" class="form-select">';
        $html .= '<option value="">Select Company Sub Category</option>';
        $html .= '</select>';
