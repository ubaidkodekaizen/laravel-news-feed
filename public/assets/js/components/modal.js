...
$(document).ready(function () {
    ...
    // Open post modal
    $("#openPostModal, .addPhoto, .addVideo, #createFirstPost").click(
        function () {
    ...
    $("#mediaUpload").on("change", function (e) {
        ...
        files.forEach((file) => {
            ...
            if (file.size > 10 * 1024 * 1024) {
                showNotification(
                    `${file.name} exceeds 10MB limit and was skipped.`,
                    "error"
                );
                return;
            }
    ...
    $("#postForm").submit(function (e) {
        ...
        const commentsEnabled = $("#commentsEnabledToggle").is(":checked")
            ? 1
            : 0;
        ...
        if (content.length > 10000) {
            showValidationError(
                "Post content is too long. Maximum 10,000 characters allowed."
            );
            return;
        }
        ...
        const oversizedFiles = window.mediaList.filter(
            (m) => m.size > 10 * 1024 * 1024
        );
        if (oversizedFiles.length > 0) {
            showValidationError(
                "Some files exceed the 10MB size limit. Please remove them and try again."
            );
            return;
        }
...
function renderMediaEditor() {
    ...
    if (window.mediaList.length === 0) {
        $("#mainMediaPreviewContainer #noMediaPlaceholder").removeClass(
            "d-none"
        );
        return;
    }
